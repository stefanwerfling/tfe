...
export class IPConnection {
    ...
    constructor() {
        ...
        this.mergeBuffer = Utils.buffer_wrapper(0);
        this.taskQueue = [];
     ...
     * @param errorCallback
     */
    disconnectInternal(errorCallback) {
     ...
     * @param errorCallback
     */
    connect(host, port, errorCallback) {
     ...
     * @param errorCallback
     */
    connectInternal(host, port, errorCallback) {
        if (this.isConnected) {
            if (errorCallback) {
                /*
                 * Not using `this.` for the error callback function because
    ...
    handleConnect() {
        ...
        if (this.registeredCallbacks[IPConnection.CALLBACK_CONNECTED] !== undefined) {
            const connectReasonCb = this.registeredCallbacks[IPConnection.CALLBACK_CONNECTED];
            connectReasonCb(connectReason);
        }
        this.disconnectProbeIID = setInterval(this.disconnectProbe.bind(this), IPConnection.DISCONNECT_PROBE_INTERVAL);
        this.popTask();
    }
    /**
     * handleIncomingData
    ...
    handleConnectionError(error) {
        ...
        if (error.errno !== undefined && error.errno === 'ECONNRESET') {
            ...
            if (this.registeredCallbacks[IPConnection.CALLBACK_DISCONNECTED] !== undefined) {
                const disconnectCb = this.registeredCallbacks[IPConnection.CALLBACK_DISCONNECTED];
     ...
     * @param error
     */
    handleAutoReconnectError(error) {
     ...
     * @param packetFID
     */
    getFunctionIDFromPacket(packetFID) {
     ...
     * @param packetSeq
     */
    getSequenceNumberFromPacket(packetSeq) {
     ...
     * @param packetPayload
     */
    getPayloadFromPacket(packetPayload) {
     ...
     * @param checkValidity
     */
    sendRequest(sendRequestDevice, sendRequestFID, sendRequestData, sendRequestPackFormat, sendRequestExpectedResponseLength, sendRequestUnpackFormat, sendRequestReturnCB, sendRequestErrorCB, startStreamResponseTimer, checkValidity) {
        ...
        if (checkValidity) {
            sendRequestDevice.checkValidity(() => {
                this.sendRequestInternal(sendRequestDevice, sendRequestFID, sendRequestData, sendRequestPackFormat, sendRequestExpectedResponseLength, sendRequestUnpackFormat, sendRequestReturnCB, sendRequestErrorCB, startStreamResponseTimer);
            }, sendRequestErrorCB);
        }
        else {
            this.sendRequestInternal(sendRequestDevice, sendRequestFID, sendRequestData, sendRequestPackFormat, sendRequestExpectedResponseLength, sendRequestUnpackFormat, sendRequestReturnCB, sendRequestErrorCB, startStreamResponseTimer);
     ...
     * @param startStreamResponseTimer
     */
    sendRequestInternal(sendRequestDevice, sendRequestFID, sendRequestData, sendRequestPackFormat, sendRequestExpectedResponseLength, sendRequestUnpackFormat, sendRequestReturnCB, sendRequestErrorCB, startStreamResponseTimer) {
        // Packet creation
        const sendRequestPayload = this.pack(sendRequestData, sendRequestPackFormat);
        const sendRequestHeader = this.createPacketHeader(sendRequestDevice, 8 + sendRequestPayload.length, sendRequestFID, sendRequestErrorCB);
            ...
            if (startStreamResponseTimer) {
                ...
                if (sendRequestFID in sendRequestDevice.streamStateObjects) {
                    ...
                    sendRequestDevice.streamStateObjects[sendRequestFID].responseProperties.timeout =
                        setTimeout(this.sendRequestTimeoutStreamOut.bind(this, sendRequestDevice, sendRequestFID, sendRequestErrorCB), this.timeout);
                }
            }
            else {
                sendRequestDevice.expectedResponses.push({
                    ...
                    timeout: setTimeout(this.sendRequestTimeout.bind(this, sendRequestDevice, sendRequestDeviceOID, sendRequestErrorCB), this.timeout),
     ...
     */
    sendRequestTimeout(timeoutDevice, timeoutDeviceOID, timeoutErrorCB) {
     ...
     */
    sendRequestTimeoutStreamOut(timeoutDevice, timeoutFID, 
    ...
    handleResponse(packetResponse) {
        ...
        if (handleResponseFID in handleResponseDevice.streamStateObjects) {
            ...
            streamStateObject.responseProperties.responseHandler(handleResponseDevice, handleResponseFID, packetResponse);
        }
        else {
            ...
            for (let i = 0; i < handleResponseDevice.expectedResponses.length; i++) {
                ...
                if (expectedResponse.FID === handleResponseFID &&
                    ...
                    if (expectedResponse.returnCB !== undefined) {
                        const retArgs = this.unpack(this.getPayloadFromPacket(packetResponse), expectedResponse.unpackFormat);
     ...
     */
    getRandomUInt32(returnCallback) {
        crypto.randomBytes(4, (error, buffer) => {
            if (error) {
                crypto.pseudoRandomBytes(4, (terror, tbuffer) => {
                    if (terror) {
     ...
     * @param errorCallback
     */
    authenticateInternal(secret, returnCallback, errorCallback) {
     ...
     * @param errorCallback
     */
    authenticate(secret, returnCallback, errorCallback) {
        this.pushTask(() => {
            if (this.nextAuthenticationNonce === 0) {
                this.getRandomUInt32((r) => {
     ...
     * @param headerErrorCB
     */
    createPacketHeader(headerDevice, headerLength, headerFunctionID, headerErrorCB) {
        ...
        if (headerDevice !== undefined) {
            const responseExpected = headerDevice.getResponseExpected(headerFunctionID, (errorCode) => {
                returnOnError = true;
     ...
     * @param chunkPadding
     */
    createChunkData(data, chunkOffset, chunkLength, chunkPadding) {
        let chunkData = data.slice(chunkOffset, chunkOffset + chunkLength);
        if (chunkData.length < chunkLength) {
            const padding = new Array(chunkLength - chunkData.length);
...
IPConnection.CONNECTION_STATE_PENDING = 2;
